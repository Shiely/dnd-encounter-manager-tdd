...
def test_tie_step1_higher_dex_wins():
    ...
    orc = MonsterDefinition(
        id="orc", name="Orc", size="Medium", type_="humanoid", alignment="chaotic evil",
        armor_class=13, hit_points=15, hit_dice="2d8+2", speed={"walk": 30},
        ability_scores=AbilityScores(16, 12, 14, 7, 11, 10),  # dex=12 → +1
        challenge_rating=ChallengeRating("1/2"), xp=100
    )
    entities = [
...
def test_tie_step3_higher_cr_wins():
    ...
    high_cr = MonsterDefinition(
        id="ogre", name="Ogre", size="Large", type_="giant", alignment="chaotic evil",
        armor_class=11, hit_points=59, hit_dice="7d10+21", speed={"walk": 40},
        ability_scores=AbilityScores(19, 8, 16, 5, 7, 7),
        challenge_rating=ChallengeRating("2"), xp=450
    )
    entities = [
        EncounterEntity("e1", "Goblin #1", "monster", 12, monster_id="goblin"),
